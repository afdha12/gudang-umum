...
@section('title', 'Detail Pending - ' . ($stationery->nama_barang ?? 'Barang'))
...
@section('content')
    <div class="container-fluid">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            <div>
                <a href="{{ route('pending.index', ['view' => 'item']) }}" class="text-decoration-none text-muted small">
                    <i class="bi bi-arrow-left me-1"></i>Kembali ke Monitoring
                </a>
                <h4 class="fw-bold text-dark mb-0 mt-1">
                    <i class="bi bi-box-seam me-2"></i>Permintaan Pending: <span class="text-uppercase">{{ $stationery->nama_barang }}</span>
                </h4>
                <p class="text-muted mb-0 small">Kode: {{ $stationery->kode_barang }} | Stok: {{ $stationery->stok }} {{ $stationery->satuan ?? 'pcs' }}</p>
            </div>
        </div>
                        ...
                        <tbody>
                            @forelse ($items as $key => $item)
                                ...
                                <tr>
                                    ...
                                    <td class="py-3 px-4">
                                        <div class="fw-semibold text-capitalize">{{ $item->user->name ?? '-' }}</div>
                                ...
                                <tr>
                                    <td colspan="6" class="py-5 text-center">
                                        <div class="text-muted">
                                            ...
                                            <span>Tidak ada permintaan pending untuk barang ini</span>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
